...
- VS Code 확장
  ...
  - 설치. Debugger 포함한 6개의 확장이 추가 설치됨
...
9. Gradle 확장이 있는지 확인. 코끼리 아이콘이 안 나타나면 Gradle이 제대로 설정 안된 것.
...
- 사용가능한 곳
  ...
  - 순수 안드로이드 개발
  - 이외 모든 분야에서 사용가능
  - 단, 시스템 프로그래밍, 높은 성능 요구, iOS 모바일 개발은 어려움
...
- 중요 용어
  - JDK : Java Development Kit. 자바 개발+실행하려면 필수로 설치해야하는 키트
  ...
  - JVM : Java Virtual Machine. 자바로 컴파일된 바이트코드를 실행시키는 가상머신. JRE/JDK 포함
    - OS 플랫폼 독립적으로 동작. 윈도우에서 개발한 자바도 리눅스 등에서 실행 가능

#### 프로젝트 그룹

- 번호가 작을수록 큰 그룹

1. VS Code 개발툴 : 개발을 위한 모든 것이 포함됨
2. Java : 개발하기 위한 언어
3. JDK : 개발 툴킷
...
- Maven : 오래된 빌드 도구. xml 기반. pom.xml로 관리
- `Gradle` : Maven 단점을 잡아낸 빌드 도구. 텍스트 문서 기반. build.gradle로 관리
...
- `src/main/java` : Java에서 개발할 실제 소스가 위치한 폴더

- src/text/java : 테스트 코드 위치한 폴더. Java는 코딩과 테스트 동시에 진행

- gradlew, gradlew.bat : Gradle Wrapper 실행하는 파일

- DSL : Domain Specific Language. 특정 분야에 최적화된 프로그래밍 언어. Gradle 설정하는 언어를 지정
  - `Groovy` : build.gradle로 파일 생성. 간결하게 사용 가능. 작성 방법이 간단
  - Kotlin : build.gradle.kts로 파일 생성. 생산성, 코드 안정성이 강점
...
- 논리연산자 : &&, ||, !
- 비트연산자 : &, |, ^
- 삼항연산자 : (a > 5) ? "크다" : "작다" . if ~ else를 inline으로 만든 연산자
- 문자열 연산자 : "Hello" + "World"
- 연산자 우선순위 : () > ++|-- > \* / % > + - > 비교연산 > 동등비교
...
- 객체지향으로 해결하자 - [소스](./day02/ex04_OOP/app/src/main/java/ex04_oop/App.java)
  ...
  students.add(new Student("성규", 35, 100));
  students.add(new Student("우현", 34, 95));
  students.add(new Student("명수", 33, 90));
  ```
...
- 캡슐화(Encapsulation)
  ...
  ```java
        ...
        // myAcc.balance = 100000000;  // 누구나(개발자) 잔고라는 멤버변수에 접근가능
        myAcc.deposit(2000000);

        System.out.println("계좌 잔고는 : " + myAcc.getBalance());
        myAcc.withdraw(30000000); // 잘못된 동작을 막아줌
        System.out.println("계좌 잔고는 : " + myAcc.getBalance());
    }
  }
  ...
  - static
    - 정적. 객체마다 따로 존재하는 것이 아니라 클래스 하나만 존재하는 멤버
...
- @Override : 부모클래스에 같은 메서드 있는거 아는데, 내가 다시 만든거 써!
- 어노테이션 종류
  - @Override
  - @Deprecated : 이제는 사용 안할 메서드 지정
...
- 객체를 직접 만들지 않고, 공통 규칙과 공통 기능을 제공하는 클래스
  ...
  abstract class Animal {
    ...
    // 추상메서드
    abstract void sound();  // 내부 구현을 하지 않는다! 하지만 자식에서는 구현하라!
...
- 비정상적 종료를 막기 위해서 안전하게 처리하고 흐름을 제어하는 방법
  ...
  - 얕은 복사
    ...
    Student s2 = s1; // 얕은 복사
    s2.name = "동우";
    System.out.println(s1.name);  // 동우 출력
    System.out.println(s2.name);  // 동우 출력
    ```
  ...
  - 깊은 복사
    ...
    System.out.println(s3.name); // 우현 출력
    System.out.println(s4.name); // 성열 출력
    ```
...
- 예제
  ...
  #### 일반화 프로그래밍
  - 일반화 필요성
    - Python, JavaScript는 데이터타입 지정없음 -> 아무런 데이터나 변수가 들어감
  ...
  - Generic(일반화) 방식 - [소스](./day03/ex06_generic/app/src/main/java/ex06_generic/App.java)
    ...
    Pair<String, Integer> p1 = new Parir<>();
    Pair<Integer, Float> p2 = new Parir<>();
