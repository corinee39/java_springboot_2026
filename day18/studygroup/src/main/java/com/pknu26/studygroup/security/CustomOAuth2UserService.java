...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 로그인 제공자가 누구인지 식별(Google, KaKao, Naver...)
        ...
        User user;  // {} local scope 때문에 미리 생성
        ...
        if (userSocialAccount != null) {  // 이미 소셜계정이 존재
            ...
            if (user == null) {  // 기존 사용자도 없음, join 처리
                Long userId = userMapper.nextUserId();  // User_Account 테이블의 PK 값을 가져옴

                user = new User();
                user.setUserId(userId);
                user.setLoginId(email);  // LoginId는 email 처리!!
                user.setPassword(null);  // 소셜로그인은 패스워드를 직접 처리하지 않음!
                user.setName(name);
                user.setRole("ROLE_USER");

                // 260430. insertSocialuser -> insertSocialUser로 변경
         ...
         // 260430. DefaultOAuth2UserDetail -> CustomOAuth2UserDetails로 변경해야 세션 저장
