...
public class PageResponse<T> {
    ...
    private final List<T> dtoList;  // 게시글 리스트 T : Board, Student, Reply 등 모두 가능
    private final int totalCount;   // 전체 게시글 수. 예) 32
    private final int currentPage;  // 사용자가 보고 있는 현재 페이지 번호
    private final int size;         // 한 페이지당 게시글 수
    ...
    */
    private final int startPage;    // 시작 페이지, 화면에 보여줄 시작페이지
    private final int endPage;      // 화면에 보여줄 마지막페이지
    private final boolean prev;     // 이전버튼 표시 여부
    private final boolean next;     // 다음버튼 표시 여부
