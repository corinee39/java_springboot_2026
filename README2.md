...
- 구성 3단계
  - 웹브라우저(프론트엔드) - 사용자가 `요청`하고 결과를 돌려받는 화면. HTML/CSS/JS
  - 웹서버(백엔드) - 사용자 요청을 받아서 DB에서 데이터를 읽고, 프론트엔드에 보낼 데이터를 전송(`응답`)
...
- 웹 기술 히스토리
  ...
  - `JSP`(Java Server Page) : HTML과 Java 소스를 분리. 쉽게 개발하도록 만든 기술(개발 난이도 중)
    - 개발환경 구성 난이도가 높음
  - `Spring` : Java 개발 전성기. 웹페이지와 Java 영역 분리. 개발환경 구성 난이도 줄어듬
    - 개발환경 구성 난이도 중
    - 대한민국 전자정부 웹프레임워크 개발
  - `Spirng Boot` : Spring 개발환경 구성 단점, 개발 단점 최소화

#### Spring Boot

- https://spring.io/ 공식 웹사이트
- Spirng의 기술 그대로 사용(마이그레이션 간단)
- JPA 기술 사용, ERD나 DB 설계 하지않고 손쉽게 DB 생성, 연동 쉬움
...
- Visual Studio Code 확장
  ...
  - Spirng Initializr Java Support 설치 확인
...
1. 명령 팔레트(Ctrl + Shift + P)에서 `Spring Initializr` 검색
...
4. Input Group Id
   - 그룹 아이디
   - com.example : mail.naver.com / news.naver.com 등에서 naver.com을 거꾸로 사용하는 것
   - mail, news : 프로젝트 ID
   - `com.pknu26`으로 통일
...
- build.gradle : 자바 버전, 플러그인, 의존성 설정 파일 - [소스](./day03/demo/build.gradle)
  ...
  version = '0.0.1-SNAPSHOT'  // 프로젝트 버전. 실제 배포 시는 1.0.0 으로 변경

  java {
      toolchain {
          languageVersion = JavaLanguageVersion.of(21)  // 사용중인 Java JDK 버전
      }
  }
...
- DemoApplication.java - [소스](./day03/demo/src/main/java/com/pknu26/demo/DemoApplication.java)
  - @SpringBootApplication : 스프링부트 설정 클래스임을 지칭. 컴포넌트 스캔 수행. 자동설정 어노테이션
  - SpringApplication.run(...) : 스프링 컨테이너 실행, 내장 서버(Tomcat) 띄움
...
- Spring MVC 구조
  ...
  ┃ ┃    ┣ SpringmvcApplication.java   // 전체 애플리케이션
  ...
  - 브라우저 요청 -> Controller 호출 -> Model을 데이터에 담고(Service가) -> View 반환 -> 요청한 브라우저에 돌려줌
...
- 어노테이션 목록
  ...
  - @ResponseBody : 응답페이지에 텍스트 출력 어노테이션, 자바의 String 문자열을 자동 생성된 웹페이지에 렌더링 작업
...
- 방법
  ...
  - `thymeleaf` 페이지 : html로 사용. 템플릿 방식. jsp 페이지처럼 URL을 공개하지 않는 방식 사용
...
- 컨트롤러 컴포넌트를 구체화 해당클래스 IoC 컨테이너에 Bean(관리되는 클래스) 등록

##### @RestController

- Restful API 개발시 사용하는 컨트롤러
...
- 각각의 메서드별 매핑이 존재해서 현재는 많이 사용 안함
- 전체 컨트롤러의 동일한 URL페이지명을 통합할 때 자주 사용
- 사용 방법
...
- URL 뒤에 ? 뒤쪽에 위치, 각 값은 key=value 구분자는 &
...
- Thymeleaf 초기설정 : 안써도 동작하지만, 쓰는 걸 습관화할 것
...
- `th:href="@{/students/list}"` : a 태그의 링크과 동일
...
- build.gradle 의존성(라이브러리)이 다운로드 후 .gradle 폴더 하위에 설치
...
- .vscode : VS Code가 프로젝트에 필요한 설정을 담는 폴더
...
- `src/main/resources` : 자바 이외의 HTML, CSS, JS, 이미지 파일, 환경 파일 등 리소스가 되는 파일 저장하는 폴더
  - static : 정적 HTML 파일, CSS, JS, 이미지 파일 저장
  - templates : 스프링부트와 연계되는 thymeleaf html이나 mustache 파일 저장
  - `application.properties` : 프로젝트 환경설정, DB설정, 환경변수
- src/test/java : JUnit 테스트 도구 자바파일 설정
...
- Choose dependencies 선택할 의존성 - [소스](./day05/webboard/build.gradle)
  - Spring Boot DevTools : 개발시 필요한 툴 기능 포함
  - Lombok : Getter/Setter 자동 생성 라이브러리
  - Spring Web : 웹사이트 관련 작업 시 필요
...
3. JPA 설정
   -Java Persistence API : 자바 관계형 데이터베이스 핸들링 방식 중 ORM 기술 사용 라이브러리
   - ORM : 쿼리를 실행하지 않고 DB와 Java 간에 데이터를 자동으로 매핑하는 기술
...
4. 필요한 controller, entity, repository, service 패키지(폴더) 생성 - [폴더](./day05/webboard/src/main/java/com/pknu26/webboard/)
...
9. 테스트는 웹서버 중지 상태에서 실행할 것 - [소스](./day05/webboard/src/test/java/com/pknu26/webboard/WebboardApplicationTests.java)
...
- @Autowired : spring Boot 핵심 철학인 의존성 주입(DI)을 자동으로 해주라는 의미
  - 생성자를 직접 구현하면 @Autowired가 필요 없음

- @RequiredArgConstructor : 파라미터 생성자를 자동으로 생성해주나, final 또는 @NotNull로 지정된 변수만 포함시켜 생성
...
- ~~의존성 필요~~
  ...
  - ~~build.gradle 추가~~
  - Spring 4.x 에서 thymeleaf layout 라이브러리가 제대로 동작 안함
...
- layout.html에 리소스 태그 추가 - [소스](./day06/webboard/src/main/resources/templates/layout.html)
...
2. validation 폴더 생성 - [폴더](./day06/webboard/src/main/java/com/pknu26/webboard/validation/)
...
- resources/mapper/Studentmapper.xml 생성, 쿼리문 작성. 빌드 후 class로 변경됨
...
- resources/mapper/BoardMapper.xml 쿼리 작성 - [소스](./day08/studygroup/src/main/resources/mapper/BoardMapper.xml)
- service/BoardService 인터페이스 - [소스](./day08/studygroup/src/main/java/com/pknu26/studygroup/service/BoardService.java)
- service/BoardServiceImpl 클래스 - [소스](./day08/studygroup/src/main/java/com/pknu26/studygroup/service/BoardServiceImpl.java)
  ...
  2. layout.html 수정
     ...
     >
       ...
       <body>
         ...
         <!-- 페이지 개별 스크립트는 맨 마지막 -->
         <script th:replace="~{layout :: layout(~{::content}, ~{})}"></script>
       </body>
     </html>
     ```
